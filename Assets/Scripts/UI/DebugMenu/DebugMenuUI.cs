    ...
    private void MainWindowUI(int windowId)
    {
        ...
        { // Main Window Area
            ...
            GUILayout.BeginVertical();
            {
                ...
                GUILayout.BeginHorizontal();
                {
                    // Create a label for the currency
                    GUILayout.Label("Currency: ", GUILayout.Width(WINDOW_DIMENSION.x / 4.0f));

                    // Create a temporary variable to hold the currency value
                    var currency = InventoryManager.Instance.availableCurrency;

                    // Represent the currency using a horizontal slider
                    currency = (int)GUILayout.HorizontalSlider(currency, 0.0f, 1000.0f, GUILayout.ExpandWidth(true));

                    // Update the currency value if it has changed
                    if (GUI.changed)
                    {
                        InventoryManager.Instance.availableCurrency = currency;
                    }
                ...
                GUILayout.BeginVertical();
                {
                    GUILayout.BeginHorizontal();
                    {
                        ...
                        GameManager.Instance.interactiveMode =
                            GUILayout.Toggle(GameManager.Instance.interactiveMode, "Enabled");
                    }
                    GUILayout.EndHorizontal();
                    ...
                    // Control for SoundManager.Instance.masterVolume
                        ...
                        SoundManager.Instance.masterVolume = GUILayout.HorizontalSlider(SoundManager.Instance.masterVolume, -80.0f, 20.0f, GUILayout.ExpandWidth(true));
                    // Control for SoundManager.Instance.masterMuted
                        if (GUILayout.Button("M", GUILayout.Width(50), GUILayout.Height(30)))
                        {
